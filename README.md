...
GitHub Release 页面里如果你还看到 `Source code (zip)` 和 `Source code (tar.gz)`，那是 GitHub 针对 tag 自动生成的系统下载项，不是这个项目主动发布的资产，也不能关闭。
...
这里的前提很重要：`crl` 需要在目标项目目录里运行。因为它会基于当前目录去发现当前工作区可恢复的 Codex 会话。如果不在相应项目目录里执行，就不会匹配到你要继续的会话。
...
如果勾选，会一并删除本机上的 Codex-Resume-Loop 配置状态和日志。
