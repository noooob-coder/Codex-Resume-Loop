...
这里的前提很重要：`crl` 需要在目标项目目录里运行。因为它会基于“当前目录”去发现当前工作区可恢复的 Codex 会话。如果不在相应项目目录里执行，就不会匹配到你要继续的会话。

桌面端启动入口：

- 开始菜单里的 `Codex-Resume-Loop Desktop`
...
如果你只想直接开始，执行 `crl` 即可。如果你已经明确知道要跑几轮，再写成：

```bash
crl 3 "继续上一次结束的位置，完成未完成的工作。"
```

提示词包装行为是真实存在的，不是文档臆测。

Codex-Resume-Loop 不会把原始提示词直接裸发给 Codex，而是先包装成更严格的恢复提示。代码里这个行为在 `build_resume_prompt()` 里实现。包装后的要求包括：
